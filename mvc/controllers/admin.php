...
class admin extends controller
{
    ...
    function addcategory()
    {
        ...
        if (isset($_POST["submit"])) {
            ...
            if (($this->categorymodel->getCategoryName($name)) >= 1) {
                $mess2 = "Đã trùng tên danh mục";
            } else if (($this->categorymodel->AddCategory($name, $publish, $slug)) == true) {
    ...
    function editcategory()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($this->categorymodel->getCategoryName($name) >= 1) {
                ...
                if ($this->categorymodel->EditCategory($name, $slug, $id) != null) {
                    $mess = "Sửa danh mục thành công";
                }
            }
        }
    ...
    function addproduct()
    {
        ...
        if (isset($_POST["submit"])) {
            ...
            if ($product["price"] < 0) {
                $notifi["price"] = "Vui Lòng Nhập Giá Lớn Hơn 0";
            }
            if ($product["price"] == "") {
            ...
            if ($product["quantity"] < 0) {
                $notifi["quantity"] = "Vui Lòng Nhập Số Lượng Lớn Hơn 0";
            }
            if ($product["sale"] == "") {
            ...
            if ($notifi == null) {
                if (($this->productmodel->GetProductName($product["name"])) >= 1) {
                    $notifi["name"] = "Đã trùng tên sản phẩmn";
                } else if (($this->productmodel->AddProduct($product["name"], $product["price"], $img_product, $product["quantity"], $product["decs"], $product["id_category"], $name_category[0]["name"], $product["sale"])) == true) {
    ...
    function editproduct()
    {
        ...
        if (isset($_POST["submit"])) {
            ...
            if ($notifi == null) {
                ...
                if (($this->productmodel->GetProductName($editproduct["name"])) >= 2) {
                    $notifi["name"] = "Đã trùng tên sản phẩmn";
                } else if (($this->productmodel->UpdateProductById($id, $editproduct["name"], $editproduct["price"], $img_product, $editproduct["quantity"], $editproduct["decs"], $editproduct["id_category"], $name_category[0]["name"])) != null) {
    ...
    function orderdetails()
    {
        ...
        $info_user1 = $this->ordermodel->GetInfoUserById($id_user);
        $info_user = $this->ordermodel->GetOrderProduct($id_user,$id_order);
