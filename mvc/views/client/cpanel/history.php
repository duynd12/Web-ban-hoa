...
<body>
	<header id="header">
					...
					<div class="col-md-6 clearfix mob-ipa">
						<div class="shop-menu clearfix pull-right">
							<ul class="nav navbar-nav collapse navbar-collapse" style="position: relative;">
								...
								<li><?php if (isset($_SESSION["info"]["name"])) echo '<a href="logout/logout"><i class="fas fa-sign-out-alt"></i>Đăng Xuất</a>';
						...
						<table class="table table-condensed">
							<thead>
								<tr class="cart_menu">
									...
									<td class="total" style = "text-align:center">Xử lí</td>
									<td></td>
								</tr>
							</thead>
							<tbody>
								...
								<?php foreach ($data["order"] as $key => $values) { ?>
									<form method="post" id="form">
										...
										<tr>
											<td class="cart_price">
												<p style="margin-bottom: 10px;"><?= $values["address"] ?>
												<p>
											</td>
											...
											<td class="cart_price">
												<p style="margin-top: 10px;" class="cart_price-p"><?= number_format($values["total_mony"], $decimals = 0, $dec_point = ",", $thousands_sep = ".") ?></p>
											</td>
		...
		<script>
			...
			var cart_price = document.querySelectorAll('.cart_price-p');
			for (var i = 0; i < cart_price.length; i++) {
				cart_price[i].innerHTML = cart_price[i].innerHTML + ' đ';
			}
		</script>
</body>
